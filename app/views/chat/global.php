...
<div class="container-fluid d-flex flex-column p-0" id="messages-container" style="margin-top:60px !important;"><h1>Loading messages...</h1></div>
...
<script>
  ...
  function getMessages(){
    ...
    .then(data => {
      ...
      data.forEach(v=>{
        content+= `
        <div class="border border-primary mt-2 w-50 bg-primary rounded p-1 text-white align-self-${v.id==<?=$_SESSION["id"]?>?'end':'start'}" style="word-wrap:break-word">
  ...
  function send(){
    fetch("<?=BASEURL?>/chat/send/0/"+encodeURIComponent(msgInput.value))
    .then(()=>msgInput.value="");
  }
</script>
